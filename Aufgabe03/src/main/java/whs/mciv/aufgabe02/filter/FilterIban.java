...
public class FilterIban implements UnaryOperator<TextFormatter.Change> {
    ...
    public static final String IBAN_REGEX = "[a-zA-Z]{0,2}\\d{0,2}\\s?(?:\\d{0,4}\\s?){0,3}\\d{0,4}(?:\\s?\\d{0,2})";
    // Regex für schlussendliche Prüfung
    public static final String IBAN_REGEX_FINAL = "[a-zA-Z]{2}\\d{2}\\s?(?:\\d{4}\\s?){3}\\d{4}(?:\\s?\\d{2})";
